...
export default function SlotsScreen() {
  ...
  const renderSlotGrid = () => (
    <ScrollView 
      ...
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={handleRefresh} colors={['#F1AA9B']} />
      }
    >
      ...
      <View style={styles.filterSection}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <Text style={[styles.sectionHeading, { marginBottom: 0 }]}>Configure Location & Event</Text>
          {(selectedDistrict || selectedMandal || selectedEvent || selectedLocalSlot || slotSearchQuery) ? (
            <TouchableOpacity onPress={handleClearForm} style={{ padding: 4 }}>
              <Text style={{ color: '#EF4444', fontWeight: '600', fontSize: 14 }}>Clear Form</Text>
            </TouchableOpacity>
          ) : null}
        </View>
