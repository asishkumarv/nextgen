...
const SLOT_SIZE = (width - 64 - 36) / 6; // 6 slots per row, padded layout

export default function SlotsScreen() {
  const insets = useSafeAreaInsets();
  const { bookedSlot, bookingDetails, bookSlot, cancelSlot } = useApp();
  const [selectedLocalSlot, setSelectedLocalSlot] = useState(null);

  // Set of dummy booked slots matching the screenshot (1, 2, 4, 5, 7, 8, 9, 10, 11, etc. are booked/pink)
  const bookedSlotsList = [
    1, 2, 4, 5, 7, 8, 9, 10, 11, 14, 15, 16, 17, 18, 19, 20, 22, 23, 24, 25,
    26, 27, 28, 29, 30, 33, 35, 37, 40, 41, 42, 44, 47, 48, 50, 51, 52, 54,
    55, 56, 57, 60
  ];

  // Generate 60 slots for display
  const totalSlots = Array.from({ length: 60 }, (_, i) => i + 1);

  const handleSelectSlot = (slotNum) => {
    if (bookedSlotsList.includes(slotNum)) {
    ...
    >
      ...
      >
        ...
        <View style={styles.progressLabelRow}>
          <Text style={styles.progressText}>1876 booked</Text>
          <Text style={styles.progressText}>1123 left</Text>
        </View>
      </LinearGradient>
      ...
      <View style={styles.gridCard}>
        <View style={styles.gridHeader}>
          <Text style={styles.gridHeading}>Showing 1 – 600 of 2999</Text>
          <Text style={styles.gridActionText}>Tap to select</Text>
        </View>

        <View style={styles.gridBody}>
          {totalSlots.map((num) => {
            const isBooked = bookedSlotsList.includes(num);
