...
export default function LoginScreen({ onNavigateToRegister }) {
  ...
  const handleSendResetOtp = async () => {
    ...
    setPhoneOrEmailError('');
    if (!phoneOrEmail.trim()) {
      setPhoneOrEmailError('Registered partner phone number is required');
      return;
    }

    setResetLoading(true);
    try {
      const res = await api.post('/auth/send-otp', { phoneOrEmail: phoneOrEmail.trim(), type: 'vendor', action: 'reset' });
            ...
            {isForgotPassword ? (
              <>
                ...
                <Text style={styles.formSub}>
                  {resetOtpSent ? 'Enter the SMS code sent to your phone and your new password' : 'Enter your registered partner phone number to receive an SMS OTP'}
                </Text>
